...
public class ProviderManageController extends BaseAdminController{
	...
	@RequestMapping(value = {"/admin/chi-tiet-danh-muc/{id}"},method = RequestMethod.GET)
	public ModelAndView DetailProvider(@PathVariable int id) {
		try {
			Provider list = providerServiceImpl.SerchProviderByID(id).get(0);
			_mvShare.setViewName("admin/provider/detailprovider");
			_mvShare.addObject("detailprovider", list);
		} catch (Exception e) {
		}
