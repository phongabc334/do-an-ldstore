...
public class UserController extends BaseController{
	...
	public ModelAndView CreateUser(@ModelAttribute User user) {
		...
		if(userServiceImpl.CheckAddUser(user)) {
			...
			_mvShare.addObject("status","Đăng ký không thành công, tên tài khoản hoặc email đã tồn tại");
			  _mvShare.setViewName("user/account/register"); 	
		}else { 
	...
	public String Changepass(HttpServletRequest request, HttpSession session,@ModelAttribute ChangePassword changepassword) {
		...
		if(count == null ) {
		  ...
		  session.setAttribute("InfoUser", adminUserServiceImpl.AdminUserById(id));
		  }
		else {
