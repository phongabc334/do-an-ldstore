                  ...
                  <table id="recent-purchases-listing" class="table table-bordered">
               	...
               	<div class="row mt-4 ml-1">
					<div class="d-flex justify-content-left">
						<a class="nav-link" href="/LdStore/admin/quan-ly-hoa-don">
	                      <span class="menu-title btn btn-outline-danger btn-fw">Quay lại</span>
	                    </a>
	                    <c:if test="${ AdminUserbilldetail.status == 'Đang chờ' }">
	                         <a class="nav-link " href="<c:url value="/admin/cap-nhat-don-hang/${ AdminUserbilldetail.id }"/>">
	                           <span class="menu-title btn btn-outline-primary btn-fw">Cập nhật</span>
	                         </a>
                        </c:if>
                        <c:if test="${ AdminUserbilldetail.status == 'Đang chuẩn bị hàng' }">
	                         <a class="nav-link " href="<c:url value="/admin/cap-nhat-don-hang/${ AdminUserbilldetail.id }"/>">
	                           <span class="menu-title btn btn-outline-primary btn-fw">Cập nhật</span>
	                         </a>
                        </c:if>
                        <c:if test="${ AdminUserbilldetail.status == 'Đang giao hàng' }">
	                         <a class="nav-link " href="<c:url value="/admin/cap-nhat-don-hang/${ AdminUserbilldetail.id }"/>">
	                           <span class="menu-title btn btn-outline-primary btn-fw">Cập nhật</span>
	                         </a>
                        </c:if>
